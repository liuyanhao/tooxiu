...
<body class="no-skin">
						...
						<div class="col-xs-12">
							...
							<table id="dynamic-table" class="table table-striped table-bordered table-hover" style="margin-top:7px;">
								...
								<tr>
									...
									<c:if test="${QX.edit == 1 }">
										<!-- 按钮循环 -->
										<c:forEach items="${buttonlist}" var="lxcbutton" varStatus="vsb">
											<th class='center'>${lxcbutton.NAME }</th>
										</c:forEach>
									</c:if>
								</tr>
								</thead>
								<c:choose>
									<c:when test="${not empty roleList_z}">
										...
										<c:if test="${QX.edit == 1 }">
											...
											<c:forEach items="${buttonlist}" var="lxcbutton" varStatus="vsb">
												<!-- 关联表循环 -->
												<c:forEach items="${roleLxcbuttonlist}" var="varRb" varStatus="vsRb">
													<c:if test="${var.ROLE_ID == varRb.ROLE_ID && lxcbutton.LXCBUTTON_ID == varRb.BUTTON_ID }">
														<c:set value="1" var="rbvalue"></c:set>
													</c:if>
												</c:forEach>
												<td class='center' style="height: 20px;">
													<label>
														<input name="switch-field-1" onclick="upRb('${var.ROLE_ID}','${lxcbutton.LXCBUTTON_ID}')" class="ace ace-switch ace-switch-3" type="checkbox" <c:if test="${rbvalue == 1 }">checked="checked"</c:if> >
														<span class="lbl"></span>
													</label>
												</td>
	...
	<script type="text/javascript">
		...
		//处理按钮点击
		function upRb(ROLE_ID,LXCBUTTON_ID){
			$.ajax({
				type: "POST",
				url: "<%=basePath%>buttonrights/upRb.do?ROLE_ID="+ROLE_ID+"&BUTTON_ID="+LXCBUTTON_ID+"&guid="+new Date().getTime(),
		    	data: encodeURI(""),
				dataType:'json',
