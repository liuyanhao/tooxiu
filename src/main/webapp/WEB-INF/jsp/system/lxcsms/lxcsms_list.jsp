...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="lxcsms/list.do" method="post" name="Form" id="Form">
						...
						<table style="margin-top:5px;">
							<tr>
								...
								<td style="padding-left:20px;"><a href="lxcsms/list.do?TYPE=1"><span class="label label-<c:if test="${pd.TYPE != '2' }">success</c:if> arrowed-right arrowed-in">收信箱</span></a></td>
								<td><a href="lxcsms/list.do?TYPE=2"><span class="label label-<c:if test="${pd.TYPE == '2' }">info</c:if> arrowed-right arrowed-in">发信箱</span></a></td>
							</tr>
						</table>
						<!-- 检索  -->
							...
							<c:choose>
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											<td class='center'>
												<label class="pos-rel"><input type='checkbox' name='ids' id="${var.TO_USERNAME}" value="${var.LXCSMS_ID}" class="ace" /><span class="lbl"></span></label>
											</td>
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													<a class="btn btn-xs btn-success" title="查看" onclick="viewx('STATUS${vs.index+1}','${var.STATUS}','${pd.TYPE == '2'?'2':'1' }','${var.LXCSMS_ID}','${var.SANME_ID}');">
														<i class="ace-icon fa fa-search nav-search-icon"></i>
													</a>
													<c:if test="${QX.LXCSMS == 1 }">
													<a class="btn btn-xs btn-info" title='发送站内信' onclick="sendLxcsms('${var.TO_USERNAME}');">
														<i class="ace-icon fa fa-envelope-o bigger-120" title="发送站内信"></i>
													</a>
													</c:if>
													<c:if test="${QX.del == 1 }">
													<a class="btn btn-xs btn-danger" onclick="del('STATUS${vs.index+1}','${var.STATUS}','${pd.TYPE == '2'?'2':'1' }','${var.LXCSMS_ID}','${var.SANME_ID}');">
												...
												<div class="hidden-md hidden-lg">
													<div class="inline pos-rel">
														...
														<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
															<li>
																<a style="cursor:pointer;" onclick="viewx('STATUS${vs.index+1}','${var.STATUS}','${pd.TYPE == '2'?'2':'1' }','${var.LXCSMS_ID}','${var.SANME_ID}');" class="tooltip-success" data-rel="tooltip" title="查看">
																	<span class="green">
																		<i class="ace-icon fa fa-search nav-search-icon"></i>
																	</span>
																</a>
															...
															<c:if test="${QX.LXCSMS == 1 }">
															<li>
																<a style="cursor:pointer;" onclick="sendLxcsms('${var.TO_USERNAME}');" class="tooltip-info" data-rel="tooltip" title="发送站内信">
																	<span class="blue">
															...
															<c:if test="${QX.del == 1 }">
															<li>
																<a style="cursor:pointer;" onclick="del('STATUS${vs.index+1}','${var.STATUS}','${pd.TYPE == '2'?'2':'1' }','${var.LXCSMS_ID}','${var.SANME_ID}');" class="tooltip-error" data-rel="tooltip" title="删除">
																	<span class="red">
						...
						<table style="width:100%;">
							<tr>
								<td style="vertical-align:top;">
									<c:if test="${QX.LXCSMS == 1 }"><a title="批量发送站内信" class="btn btn-mini btn-info" onclick="makeAll('确定要给选中的用户发送站内信吗?');"><i class="ace-icon fa fa-envelope-o bigger-120"></i></a></c:if>
									<c:if test="${QX.del == 1 }">
	...
	<script type="text/javascript">
		...
		//发站内信
		function sendLxcsms(username){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="站内信";
			 diag.URL = '<%=basePath%>lxcsms/goAdd.do?username='+username;
			 diag.Width = 660;
			 diag.Height = 444;
		...
		function del(ztid,STATUS,type,Id,SANME_ID){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(type == "1" && STATUS == '2' && $("#"+ztid).html() == '<span class="label label-important arrowed-in">未读</span>'){
					top.readLxcsms();//读取站内信时减少未读总数  <!-- readLxcsms()函数在 WebRoot\static\js\myjs\head.js中 -->
				}
				if(result) {
					top.jzts();
