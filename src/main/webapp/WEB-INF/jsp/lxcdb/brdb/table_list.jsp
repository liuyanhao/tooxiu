...
<body class="no-skin">
								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											<c:if test="${dbtype != 'sqlserver' }">
											<td class='center'>
												<label class="pos-rel"><input type='checkbox' name='ids' id='lxcid${vs.index}' value="${var}" class="ace" /><span class="lbl"></span></label>
											</td>
											</c:if>
	...
	<script type="text/javascript">
		...
		//备份单表
		function backupTable(lxctable){
			bootbox.confirm("确定要备份这表[ "+lxctable+" ]吗?", function(result) {
				if(result) {
					backupAjax(lxctable,"backupts");
				}
			});
		}
		...
		* id ：提示对象的ID
		*/
		function backupAjax(lxctable,id){
			$("#backuping1").show();
			$("#backuping2").show();
				 $.ajax({
						type: "POST",
						url: '<%=basePath%>brdb/backupTable.do?tm='+new Date().getTime(),
				    	data: {lxctable:lxctable},
						dataType:'json',
						//beforeSend: validateData,
						cache: false,
						success: function(data){
				...
				if(result) {
					...
					for(var i=0;i < document.getElementsByName('ids').length;i++){
					  if(document.getElementsByName('ids')[i].checked){
					  	...
					  	else str += ',lxc,' + document.getElementsByName('ids')[i].value;
					  	
					  	if(fid=='') fid += document.getElementsByName('ids')[i].id;
					  	else fid += ',lxc,' + document.getElementsByName('ids')[i].id;
					  }
					}
					if(str==''){
						bootbox.dialog({
						...
						if(msg == '确定要批量备份这些表吗？'){
							var arrTable = str.split(',lxc,');
							var arrFid = fid.split(',lxc,');
							for(var lxci=0;lxci<arrTable.length;lxci++){
								backupAjax(arrTable[lxci],arrFid[lxci]);
							}
						}
					}
				}
