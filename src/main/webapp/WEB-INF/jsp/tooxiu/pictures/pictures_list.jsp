...
<body class="no-skin">
	<div class="main-container" id="main-container">
		...
		<div class="main-content">
			<div class="main-content-inner">
					<!-- 检索  -->
					<form action="tooxiuPictures/list.do" method="post" name="Form" id="Form">
							...
							<c:when test="${not empty varList}">
								...
								<c:forEach items="${varList}" var="var" varStatus="vs">
									<tr>
										...
										<a href="/tooxiuIamge/${var.PATH}" title="${var.TITLE}" class="bwGal"><img src="/tooxiuIamge/${var.PATH}" alt="${var.TITLE}" width="100"></a>
										</td>
	...
	<script type="text/javascript">
		$(top.hangge());
		var masterId = '${pd.MASTER_ID}';  // 服装类型 id
		//检索
		function searchs(){
			top.jzts();
			$("#Form").submit();
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>tooxiuPictures/goAdd.do?MASTER_ID='+masterId;
			 diag.Width = 800;
			 diag.Height = 490;
		...
		function del(Id,PATH){
			...
			if(confirm("确定要删除?")){ 
				top.jzts();
				var url = "<%=basePath%>tooxiuPictures/delete.do?PICTURES_ID="+Id+"&PATH="+PATH+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>tooxiuPictures/goEdit.do?PICTURES_ID='+Id;
			 diag.Width = 600;
			 diag.Height = 465;
			 diag.CancelEvent = function(){ //关闭事件
			...
			if(confirm(msg)){ 
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>tooxiuPictures/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
