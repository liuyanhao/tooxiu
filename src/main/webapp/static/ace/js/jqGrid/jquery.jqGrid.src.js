...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		setHeadCheckBox = function ( checked ) {
			...
			if(fid) {
				$('#cb_'+$.jgrid.jqID(ts.p.id),ts.grid.lxcDiv)[ts.p.useProp ? 'prop': 'attr']("checked", checked);
			}
		},
		...
		sortData = function (index, idxcol,reload,sor, obj){
			...
			if(ts.p.multiSort) {
				...
				if(ts.p.frozenColumns) {
					ts.grid.lxcDiv.find("span.ui-grid-ico-sort").addClass('ui-state-disabled');
					ts.grid.lxcDiv.find("th").attr("aria-selected","false");
				}
				...
				if(!ts.p.viewsortcols[0]) {
					if(ts.p.lastsort !== idxcol) {
						if(ts.p.frozenColumns){
							ts.grid.lxcDiv.find("span.s-ico").hide();
						}
			...
			for ( i=0;i<$t.p.colModel.length;i++ ) {
				if ( $t.p.colModel[i].index === colname || $t.p.colModel[i].name === colname ) {
					...
					if($t.p.frozenColumns === true && $t.p.colModel[i].frozen === true) {
						sobj = $t.grid.lxcDiv.find("#" + $t.p.id + "_" + colname);
					}
					break;
				}
			}
	...
	filterToolbar : function(p){
		...
		return this.each(function(){
			...
			var triggerToolbar = function() {
				...
				$.each($t.p.colModel,function(){
					var $elem = $("#gs_"+$.jgrid.jqID(this.name), (this.frozen===true && $t.p.frozenColumns === true) ?  $t.grid.lxcDiv : $t.grid.hDiv);
			...
			clearToolbar = function(trigger){
				...
				$.each($t.p.colModel,function(){
					var v, $elem = $("#gs_"+$.jgrid.jqID(this.name),(this.frozen===true && $t.p.frozenColumns === true) ?  $t.grid.lxcDiv : $t.grid.hDiv);
			...
			toggleToolbar = function(){
				...
				trow2 = $t.p.frozenColumns === true ?  $("tr.ui-search-toolbar",$t.grid.lxcDiv) : false;
	...
	setFrozenColumns : function () {
		return this.each(function() {
			...
			if( maxfrozen>=0 && frozen) {
				...
				$t.grid.lxcDiv = $('<div style="position:absolute;left:0px;top:'+top+'px;height:'+hth+'px;" class="frozen-div ui-state-default ui-jqgrid-hdiv"></div>');
				$t.grid.lxcDiv = $('<div style="position:absolute;left:0px;top:'+(parseInt(top,10)+parseInt(hth,10) + 1)+'px;overflow-y:hidden" class="frozen-bdiv ui-jqgrid-bdiv"></div>');
				$("#gview_"+$.jgrid.jqID($t.p.id)).append($t.grid.lxcDiv);
				...
				$(htbl).width(1);
				// resizing stuff
				$($t.grid.lxcDiv).append(htbl)
				.mousemove(function (e) {
					if($t.grid.resizing){ $t.grid.dragMove(e);return false; }
				});
				$($t).bind('jqGridResizeStop.setFrozenColumns', function (e, w, index) {
					var rhth = $(".ui-jqgrid-htable",$t.grid.lxcDiv);
				...
				$($t).bind('jqGridSortCol.setFrozenColumns', function (e, index, idxcol) {
					...
					var previousSelectedTh = $("tr.ui-jqgrid-labels:last th:eq("+$t.p.lastsort+")",$t.grid.lxcDiv), newSelectedTh = $("tr.ui-jqgrid-labels:last th:eq("+idxcol+")",$t.grid.lxcDiv);
	...
	destroyFrozenColumns :  function() {
		return this.each(function() {
			...
			if(this.p.frozenColumns === true) {
				var $t = this;
				$($t.grid.lxcDiv).remove();
				$($t.grid.fbDiv).remove();
				$t.grid.lxcDiv = null; $t.grid.fbDiv=null;
