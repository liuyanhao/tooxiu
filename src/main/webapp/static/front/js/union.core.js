...
adManager.adLoad = function(adMark, adName, adType, adNumber, adKey, adSize, adPrice, adTag, adRequest) {
	...
	this.adInsert = function() {
		...
		case 9:
			...
			str = '<embed src="' + this.adRequest + 'image-silde.swf" wmode="opaque" FlashVars="bcastr_file=' + _files.substr(0, _files.length - 1) + '&bcastr_link=' + _links.substr(0, _links.length - 1) + '&bcastr_title=" menu="false" quality="high" width="' + _width + '" height="' + _height + '" type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer"></embed>';
			break;
		case 10:
			for (var i = 0; i < this.adArray.length; i++) {
				var adId = this.adArray[i][0];
				...
				var adHeight = this.adArray[i][4];
				var adSTime = this.adArray[i][10];
				adManager.$V('<div id="slide-ads-' + adId + '" style="width:' + adWidth + 'px; position:absolute;">' + this.adCreate(i) + '</div>'); (function() {
					var ADId = adId;
					var ADHeight = adHeight;
					var ADWidth = adWidth;
					var ADSTime = adSTime;
