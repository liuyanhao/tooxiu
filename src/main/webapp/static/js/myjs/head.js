...
var lxcsmsCount = 0;		//站内信总数
var USER_ID;			//用户ID
var user = "LXC";		//用于即时通讯（ 当前登录用户）
var TLXCsmsSound = '1';	//站内信提示音效
...
function getHeadMsg(){
	$.ajax({
		...
		success: function(data){
			 ...
			 lxcsmsCount = Number(data.lxcsmsCount);
			 $("#lxcsmsCount").html(Number(lxcsmsCount));	//站内信未读总数
			 TLXCsmsSound = data.LXCsmsSound;				//站内信提示音效
	...
	if (window.WebSocket) {
		...
		websocket.onmessage = function(message) {
			...
			if(message.type == 'goOut'){
				...
				$("body").html("");
				goOut("2");
			}else if(message.type == 'senLxcsms'){
				lxcsmsCount = Number(lxcsmsCount)+1;
				$("#lxcsmsCount").html(Number(lxcsmsCount));
				$("#lxcsmsobj").html('<audio style="display: none;" id="lxcsmstsy" src="static/sound/'+TLXCsmsSound+'.mp3" autoplay controls></audio>');
				$("#lxcsmstss").tips({
					side:3,
		            msg:'有新消息',
		            bg:'#AE81FF',
...
//去通知收信人有站内信接收
function lxcsmsmsg(USERNAME){
	var arrUSERNAME = USERNAME.split(';');
	for(var i=0;i<arrUSERNAME.length;i++){
		websocket.send('[lxcsms]'+arrUSERNAME[i]);//发送通知
	}
}
...
function readLxcsms(){
	lxcsmsCount = Number(lxcsmsCount)-1;
	$("#lxcsmsCount").html(Number(lxcsmsCount) <= 0 ?'0':lxcsmsCount);
}
...
function lxcsms(){
	 ...
	 diag.Drag=true;
	 diag.Title ="站内信";
	 diag.URL = locat+'/lxcsms/list.do?STATUS=2';
	 diag.Width = 800;
	 diag.Height = 500;
	 diag.CancelEvent = function(){ //关闭事件
		diag.close();
