...
$(function(){if('createCode'== locat[3]){locat =  locat[0]+'//'+locat[2];}else{locat =  locat[0]+'//'+locat[2]+'/'+locat[3];};});
	...
	function save(){
		...
		for(var i=0;i<arField.length;i++){
			strArField = strArField + arField[i] + "Q1094921525";
		}
		...
		if("" != CREATECODE_ID){
			...
			$.ajax({
				...
				success: function(data){
					...
					var tb = data.pd.TABLENAME.split(",lxc,");
					$("#tabletop").val(tb[0]);
	...
	function saveD(){
		...
		var fields = dname + ',lxc,' + dtype + ',lxc,' + dbz + ',lxc,' + isQian + ',lxc,' + ddefault + ',lxc,' + flength + ',lxc,' + decimal;
	...
	function editField(value,msgIndex){
		$("#dialog-add").css("display","block");
		var efieldarray = value.split(',lxc,');
	...
	function appendC(value){
		var fieldarray = value.split(',lxc,');
		$("#fields").append(
			'<tr>'+
	...
	function isSame(value){
		for(var i=0;i<arField.length;i++){
			var array0 = arField[i].split(',lxc,')[0];
			if(array0 == value){
				return false;
			}
		}
