...
public class OnlineChatServer extends WebSocketServer{
	...
	public void onMessage(WebSocket conn, String message){
		...
		}else if(null != message && message.startsWith("[lxcsms]")){
			this.senLxcsms(message.replaceFirst("\\[lxcsms\\]", ""));
		}else if(null != message && message.startsWith("[leave]")){
			this.userLeave(conn);
		}else if(null != message && message.startsWith("[count]")){
			this.getUserCount(conn);
		}else if(null != message && message.startsWith("[QQ1094921525]")){
			OnlineChatServerPool.setLxcadmin(conn);
			this.getUserList();
		}else{
	 ...
	 * @param user
	 */
	public void senLxcsms(String user){
		JSONObject result = new JSONObject();
		result.element("type", "senLxcsms");
	...
	public void getUserList(){
		WebSocket conn =  OnlineChatServerPool.getLxcadmin();
		if(null == conn){return;}
		JSONObject result = new JSONObject();
	 ...
	 * @param conn
	 */
	public void addUserToLxcadmin(String user){
		WebSocket conn =  OnlineChatServerPool.getLxcadmin();
		if(null == conn){return;}
		JSONObject result = new JSONObject();
