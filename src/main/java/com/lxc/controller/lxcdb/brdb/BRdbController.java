package com.lxc.controller.lxcdb.brdb;
...
import com.lxc.util.DbLXC;
import com.lxc.util.FileUtil;
...
public class BRdbController extends BaseController {
	...
	public ModelAndView listAllTable() throws Exception{
		...
		Object[] arrOb = DbLXC.getTables();
		List<String> tblist = (List<String>)arrOb[1];
		mv.setViewName("lxcdb/brdb/table_list");
	...
	public Object backupAll(){
		...
		String kackupPath;
		try {
			kackupPath = DbLXC.getDbLXC().backup("").toString();		//调用数据库备份
			if(Tools.notEmpty(kackupPath) && !"errer".equals(kackupPath)){
				pd.put("LXCDB_ID", this.get32UUID());				//主键
	...
	public Object backupTable(){
		...
		Map<String,Object> map = new HashMap<String,Object>();
		pd = this.getPageData();
		String TABLENAME = pd.getString("lxctable");					//页面ajax传过来的表名
		List<PageData> pdList = new ArrayList<PageData>();
		String kackupPath;
		try {
			kackupPath = DbLXC.getDbLXC().backup(TABLENAME).toString();	//调用数据库备份
			if(Tools.notEmpty(kackupPath) && !"errer".equals(kackupPath)){
				pd.put("LXCDB_ID", this.get32UUID());				//主键
				pd.put("USERNAME", username);						//操作用户
				pd.put("BACKUP_TIME", Tools.date2Str(new Date()));	//备份时间
	...
	public Object dbRecover(){
		...
		try {
			String returnStr = DbLXC.getDbLXC().recover(TABLENAME, SQLPATH).toString();
	...
	public void delete(PrintWriter out) throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"删除Lxcdb");
	...
	public ModelAndView edit() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"修改Lxcdb");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "edit")){return null;} //校验权限
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
	...
	public ModelAndView list(Page page) throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"列表Lxcdb");
		...
		page.setPd(pd);
		List<PageData>	varList = brdbService.list(page);			//列出Lxcdb列表
		Map<String,String> DBmap = DbLXC.getDBParameter();
		mv.setViewName("lxcdb/brdb/brdb_list");
	...
	public ModelAndView goEdit()throws Exception{
		...
		mv.setViewName("lxcdb/brdb/brdb_edit");
