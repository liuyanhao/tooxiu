...
public class CreateCodeController extends BaseController {
	...
	public void proCode(HttpServletResponse response) throws Exception{
		...
		String LXCTYPE = pd.getString("LXCTYPE");  					//模块类型			========参数0-2 类型，单表、树形结构、主表明细表
		...
		for(int i=0; i< zindex; i++){
			fieldList.add(pd.getString("field"+i).split(",lxc,"));	//属性放到集合里面
		}
		...
		}else if("fathertable".equals(LXCTYPE)){
			ftlPath = "createFaCode";	//主表
		}else if("sontable".equals(LXCTYPE)){
	...
	public void save(PageData pd) throws Exception{
		...
		pd.put("TABLENAME", pd.getString("tabletop")+",lxc,"+pd.getString("objectName").toUpperCase());	//表名
