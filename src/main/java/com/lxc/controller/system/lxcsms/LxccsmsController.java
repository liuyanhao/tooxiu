package com.lxc.controller.system.lxcsms;
 ...
 */
@Controller
@RequestMapping(value="/lxcsms")
public class LxccsmsController extends BaseController {
	
	String menuUrl = "lxcsms/list.do"; //菜单地址(权限用)
	...
	public void delete(PrintWriter out) throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"删除Lxcsms");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "del")){return;} //校验权限
		PageData pd = new PageData();
		pd = this.getPageData();
		lxcsmsService.delete(pd);
		out.write("success");
		out.close();
	}
	...
	public ModelAndView list(Page page) throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"列表Lxcsms");
		...
		page.setPd(pd);	
		List<PageData>	varList = lxcsmsService.list(page);		//列出Lxcsms列表
		mv.setViewName("system/lxcsms/lxcsms_list");
	...
	public ModelAndView goAdd()throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		mv.setViewName("system/lxcsms/lxcsms_edit");
	...
	public ModelAndView goView()throws Exception{
		...
		if("1".equals(pd.getString("TYPE")) && "2".equals(pd.getString("STATUS"))){ //在收信箱里面查看未读的站内信时去数据库改变未读状态为已读
			lxcsmsService.edit(pd);
		}
		pd = lxcsmsService.findById(pd);	//根据ID读取
		mv.setViewName("system/lxcsms/lxcsms_view");
		mv.addObject("pd", pd);
		return mv;
	}	
	...
	public Object deleteAll() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"批量删除Lxcsms");
		...
		if(null != DATA_IDS && !"".equals(DATA_IDS)){
			...
			lxcsmsService.deleteAll(arrayDATA_IDS);
			pd.put("msg", "ok");
		}else{
			pd.put("msg", "no");
		}
